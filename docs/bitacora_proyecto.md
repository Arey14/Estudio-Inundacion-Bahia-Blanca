...
### 5.3. Evaluación de Subestimación, Sobreestimación y Selección del Mejor Modelo (Sin Ground Truth Absoluto)

Una de las principales dificultades científicas en este proyecto es la **ausencia de un Ground Truth absoluto** (un mapa digitalizado a mano píxel a píxel de la inundación real sobre el terreno). Sin este dato, no es posible calcular métricas de error absolutas directas. Para resolver este desafío y determinar objetivamente qué modelos subestiman o sobreestiman, empleamos criterios físicos, metodológicos y de consistencia externa:

#### 1. Metodología de Evaluación Adoptada y la "Paradoja de la Tautología"
*   **La Máscara de Consenso como Referencia:** Construimos una "verdad de referencia" mediante la intersección estricta de tres fuentes independientes: la lógica física del MNDWI (> 0.4), la estadística lineal de Random Forest Base, y la espacial convolucional de U-Net. Esto define el núcleo de agua donde existe un 100% de certeza.
*   **La Paradoja del Random Forest Base:** Como el Random Forest Base formó parte constitutiva de la regla para generar el Consenso de referencia, es metodológicamente obvio (casi una tautología circular) que sus métricas de solapamiento sean las más elevadas de la tabla (**IoU de 0.6968 y Dice de 0.8213**). Esto no significa que sea el modelo más inteligente, sino que es el origen estadístico de la propia regla de referencia. Del mismo modo, la U-Net de destilación (entrenada para imitar al RF Base) obtiene valores muy altos por correlación directa.
*   **Función Real de las Métricas IoU y Dice:** La verdadera utilidad de Jaccard y Dice no es medir el éxito del RF Base, sino evaluar cómo se desvían o aproximan los otros modelos avanzados que **no** formaron parte de ese consenso original y ver cómo generalizan espacialmente:
    *   **En modelos avanzados (Ajuste Fino):** Un Jaccard de **0.11** o **0.33** no representa una mala calidad de clasificación. Representa una **máxima sensibilidad espacial**. Al procesar texturas complejas y vecindades en parches de $224 \times 224$ píxeles, estos modelos generalizan y detectan agua líquida real en canales estrechos y planicies periurbanas que la restrictiva máscara de consenso omitió. Al detectar más agua real, la "Unión" (el denominador de Jaccard) se infla de forma correcta, reduciendo matemáticamente el solapamiento relativo del índice frente al consenso inicial pero resultando en un mapa de inundación real mucho más completo y útil.

#### 2. Modelos Evaluados como "No Tan Buenos" (Debilidades)
*   **Prithvi-EO-2.0 Zero-Shot (Sobreestimación Extrema):** Lo consideramos un modelo deficiente en su estado original. Al predecir **8,966 ha** (con DEM) y más de **10,928 ha** (sin DEM), clasifica de forma masiva tierras altas húmedas y llanuras costeras secas como inundadas. Su baja coincidencia relativa de consenso (IoU: 0.1692) se debe puramente a la inmensa cantidad de falsos positivos en su máscara.
*   **Random Forest con PCA (Subestimación Severa):** Al comprimir la información en 3 componentes principales, remueve eficazmente el ruido (diferencia de solo 63 ha con el DEM). Sin embargo, esto hace que pierda la capacidad de capturar canales finos y agua acumulada marginal en sectores urbanos bajos. Su estimación poblacional es de solo **337 personas**, ignorando el impacto real en los barrios vulnerables periféricos.

#### 3. El Modelo Seleccionado como "El Mejor" del Proyecto
*   **Prithvi-EO-2.0 (Ajuste Fino Local):** Consideramos a esta aproximación la mejor y más balanceada del estudio por las siguientes razones:
    1.  **Sensibilidad Geográfica y Poblacional Realista:** Delimita **5,869.12 ha** netas de inundación. Aunque predice más hectáreas que el consenso de base, su población afectada asociada es la más alta y realista de todas (**2,897 habitantes**). Esto demuestra que el modelo concentró su esfuerzo de clasificación en las cuencas bajas del arroyo Napostá y zonas costeras adyacentes a las áreas habitadas periféricas de alta densidad demográfica, coincidiendo espacialmente con los reportes de prensa y Defensa Civil de Bahía Blanca que informaron de múltiples anegamientos y evacuados en esos barrios específicos.
    2.  **Mitigación del Sesgo Espectral mediante el DEM:** Su diferencia con y sin DEM (**2,818.80 ha**) muestra que el modelo mantiene una alta sensibilidad espectral al agua, delegando la discriminación física final de laderas altas al DEM. Esto representa un flujo de ingeniería robusto y seguro (alta sensibilidad en el clasificador óptico y alta especificidad física en el postprocesamiento topográfico).
    3.  **Entendimiento Espacial Superior:** Al heredar los pesos pre-entrenados del Vision Transformer (ViT) de la NASA, comprende patrones estructurales a gran escala que los clasificadores pixel-a-pixel ignoran.

#### 4. U-Net Ajuste Fino como Alternativa de "Especificidad Espacial"
*   La **U-Net con Ajuste Fino** se destaca como el modelo con mayor especificidad espacial autónoma. Al reportar casi idénticas hectáreas con o sin DEM (diferencia de solo **0.84 ha**), demuestra haber asimilado internamente la topografía local de la cuenca baja, logrando predecir **4,446.60 ha** de inundación confinada a los valles sin requerir del filtro físico del DEM en el postprocesamiento. Sin embargo, estimó una población expuesta muy baja (**280 personas**) al subestimar levemente los desbordes urbanos marginales de los bordes del canal.
